...
inline dynd::nd::array make_strided_array(const dynd::ndt::type &dtp, intptr_t ndim, const intptr_t *shape)
{
...
PYDYND_API dynd::nd::array array_full(const dynd::ndt::type &d, PyObject *value, PyObject *access);
PYDYND_API dynd::nd::array array_full(PyObject *shape, const dynd::ndt::type &d, PyObject *value, PyObject *access);
...
inline bool array_is_c_contiguous(const dynd::nd::array &n)
{
  ...
  return dynd::strides_are_c_contiguous(ndim, n.get_dtype().get_data_size(), shape.get(), strides.get());
}
...
inline bool array_is_f_contiguous(const dynd::nd::array &n)
{
  ...
  n.get_shape(shape.get());
  n.get_strides(strides.get());
  return dynd::strides_are_f_contiguous(ndim, n.get_dtype().get_data_size(), shape.get(), strides.get());
}
...
inline PyObject *array_nonzero(const dynd::nd::array &n)
{
  ...
  case dynd::string_kind_id: {
    ...
    const dynd::ndt::base_string_type *bsd = n_eval.get_type().extended<dynd::ndt::base_string_type>();
  ...
  case dynd::bytes_kind_id: {
    ...
    dynd::nd::array n_eval = n.eval();
    const dynd::ndt::base_bytes_type *bbd = n_eval.get_type().extended<dynd::ndt::base_bytes_type>();
    const char *begin = NULL, *end = NULL;
...
inline void pyobject_as_irange_array(intptr_t &out_size, dynd::shortvector<dynd::irange> &out_indices,
                                     PyObject *subscript)
{
 ...
 */
inline dynd::nd::array array_getitem(const dynd::nd::array &n, PyObject *subscript)
{
 ...
 */
inline void array_setitem(const dynd::nd::array &n, PyObject *subscript, PyObject *value)
{
  if (subscript == Py_Ellipsis) {
    n.assign(value);
 ...
 */
inline dynd::nd::array array_old_range(PyObject *start, PyObject *stop, PyObject *step, PyObject *dt)
{
  ...
  if (dt != Py_None) {
    ...
    dt_nd =
        promote_types_arithmetic(start_nd.get_type(), promote_types_arithmetic(stop_nd.get_type(), step_nd.get_type()));
  }
  ...
  if (!start_nd.is_scalar() || !stop_nd.is_scalar() || !step_nd.is_scalar()) {
    throw std::runtime_error("nd::range should only be called with scalar parameters");
  }

  return dynd::nd::old_range(dt_nd, start_nd.cdata(), stop_nd.cdata(), step_nd.cdata());
}

/**
 * Implementation of nd.linspace().
 */
inline dynd::nd::array array_old_linspace(PyObject *start, PyObject *stop, PyObject *count, PyObject *dt)
{
  ...
  if (dt == Py_None) {
    ...
    return dynd::nd::old_linspace(start_nd, stop_nd, count_val, dynd_ndt_as_cpp_type(dt));
  }
}
...
inline dynd::nd::array nd_fields(const dynd::nd::array &n, PyObject *field_list)
{
  ...
  if (selected_fields.empty()) {
    throw std::runtime_error("nd.fields requires at least one field name to be specified");
  }
  ...
  dynd::ndt::type rudt = dynd::ndt::make_type<dynd::ndt::struct_type>(selected_fields, selected__types);
...
inline dynd::nd::array dynd_parse_json_type(const dynd::ndt::type &tp, const dynd::nd::array &json, PyObject *ectx_obj)
{
  return dynd::parse_json(tp, json, &dynd::eval::default_eval_context);
}

inline void dynd_parse_json_array(dynd::nd::array &out, const dynd::nd::array &json, PyObject *ectx_obj)
{
