...
#define DYND_DEFINE_PLACEMENT_WRAPPER(cpp_type, cpp_type_no_namespace,         \
                                      wrapper_type)                            \
  ...
  struct wrapper_type {                                                        \
    intptr_t                                                                   \
        dummy[(sizeof(cpp_type) + sizeof(intptr_t) - 1) / sizeof(intptr_t)];   \
  };                                                                           \
                                                                               \
  ...
  {                                                                            \
    ...
    ((cpp_type *)(&v))->~cpp_type_no_namespace();                              \
                                                                               ...
                                                                               \
  /* placement cast */                                                         \
  inline cpp_type &GET(wrapper_type &v) { return *(cpp_type *)&v; }            \
                                                                               \
  /* placement assignment */                                                   \
  inline void SET(wrapper_type &v, const cpp_type &d) { *(cpp_type *)&v = d; }
...
DYND_DEFINE_PLACEMENT_WRAPPER(dynd::vm::elwise_program, elwise_program,
                              vm_elwise_program_placement_wrapper);
