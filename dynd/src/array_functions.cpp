...
PyObject *pydynd::array_nonzero(const dynd::nd::array &n)
{
  ...
  case string_kind: {
    ...
    const char *begin = NULL, *end = NULL;
    bsd->get_string_range(&begin, &end, n_eval.get_arrmeta(),
                          n_eval.get_readonly_originptr());
    if (begin != end) {
      Py_INCREF(Py_True);
      return Py_True;
    } else {
  ...
  case bytes_kind: {
    ...
    const char *begin = NULL, *end = NULL;
    bbd->get_bytes_range(&begin, &end, n_eval.get_arrmeta(),
                         n_eval.get_readonly_originptr());
    while (begin != end) {
      if (*begin != 0) {
        Py_INCREF(Py_True);
...
bool pydynd::array_contains(const dynd::nd::array &n, PyObject *x)
{
  ...
  ckernel_builder<kernel_request_host> k;
  try {
    make_comparison_kernel(&k, 0, x_dt, x_arrmeta, child_dt, child_arrmeta,
  ...
  aux.k = &k;
  aux.found = false;
  budd->foreach_leading(arrmeta, const_cast<char *>(data), &contains_callback,
                        &aux);
  return aux.found;
}
...
{
  return DyND_PyWrapper_New(array_from_py(a, 0, false, &eval::default_eval_context)
