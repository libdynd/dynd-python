...
struct pyobject_expr_kernel_extra {
                                     ...
                                     const intptr_t *src_stride, size_t count)
  {
    ...
    for (size_t i = 0; i != src_count; ++i) {
      ndo[i + 1]->v.get_ndo()->m_data_pointer = const_cast<char *>(src[i]);
      md = reinterpret_cast<fixed_dim_type_arrmeta *>(
          ndo[i + 1]->v.get_arrmeta());
      md->dim_size = count;
  ...
  inline void verify_postcall_consistency(PyObject *res)
  {
    ...
    if (res != Py_None) {
      throw runtime_error("Python callable for elwise_map must not return a "
                          "value, got an object");
    }
                      ...
                      ckernel_prefix *extra)
  {
    ...
    extra_type *e = reinterpret_cast<extra_type *>(extra);
    // Put all the arrays in a tuple
    pyobject_ownref args(
        e->set_data_pointers(dst, dst_stride, src, src_stride, count));
    // Call the function
    pyobject_ownref res(PyObject_Call(e->callable, args.get(), NULL));
    args.clear();
    e->verify_postcall_consistency(res.get());
...
public:
  pyobject_elwise_expr_kernel_generator(PyObject *callable,
                                        ...
                                        const std::vector<ndt::type> &src_tp)
                          ...
                          const eval::eval_context *ectx) const
  {
    ...
    if (src_count != m_src_tp.size()) {
      stringstream ss;
      ss << "This elwise_map kernel requires " << m_src_tp.size()
         << " src operands, ";
    ...
    inc_ckb_offset(ckb_offset, sizeof(pyobject_expr_kernel_extra) +
                                   (src_count + 1) * sizeof(WArray *));
    reinterpret_cast<ckernel_builder<kernel_request_host> *>(ckb)
        ->reserve(ckb_offset);
    pyobject_expr_kernel_extra *e =
        reinterpret_cast<ckernel_builder<kernel_request_host> *>(ckb)
            ->get_at<pyobject_expr_kernel_extra>(root_ckb_offset);
    ...
    if (dst_tp.get_arrmeta_size() > 0) {
      dst_tp.extended()->arrmeta_copy_construct(
          n.get_arrmeta() + sizeof(fixed_dim_type_arrmeta), dst_arrmeta, NULL);
    }
    ...
    for (size_t i = 0; i != src_count; ++i) {
      ...
      if (src_tp[i].get_arrmeta_size() > 0) {
        src_tp[i].extended()->arrmeta_copy_construct(
            n.get_arrmeta() + sizeof(fixed_dim_type_arrmeta), src_arrmeta[i],
            NULL);
      }
                                    ...
                                    PyObject *dst_type, PyObject *src_type_list)
{
  ...
  for (size_t i = 0; i != n.size(); ++i) {
    ...
    if (undim_i > 0) {
      ...
      incremental_broadcast(undim, result_shape.get(), undim_i,
                            tmp_shape.get());
    }
  }
  ...
  ndt::type edt = ndt::make_expr(
      result_vdt, result.get_type(),
                             ...
                             PyObject *dst_type, PyObject *src_type)
{
  ...
  if (src_type != Py_None) {
    if (!PyList_Check(src_type)) {
      PyErr_SetString(PyExc_TypeError,
                      "Fourth parameter to elwise_map, 'src_type', "
  ...
  if (PyList_Size(n_obj) == 1) {
    return unary_elwise_map(PyList_GET_ITEM(n_obj, 0), callable, dst_type,
                            src_type == Py_None ? Py_None
                                                : PyList_GET_ITEM(src_type, 0));
  }
  else {
