...
PyObject *array_from_ptr(PyObject *tp, PyObject *ptr, PyObject *owner,
                         PyObject *access)
{
  try {
    ...
    size_t ptr_val = pydynd::pyobject_as_size_t(ptr);
    uint32_t access_flags = pydynd::pyarg_strings_to_int(
    ...
    dynd::nd::array result(dynd::make_array_memory_block(d.get_arrmeta_size()));
    if (d.get_flags() & (dynd::type_flag_destructor)) {
      stringstream ss;
    ...
    if (d.get_arrmeta_size() > 0) {
      d.extended()->arrmeta_default_construct(result.get_ndo()->get_arrmeta(),
                                              false);
    }
    d.swap(result.get_ndo()->m_type);
    result.get_ndo()->m_data_pointer = reinterpret_cast<char *>(ptr_val);
    dynd::memory_block_ptr owner_memblock =
        dynd::make_external_memory_block(owner, &pydynd::py_decref_function);
    Py_INCREF(owner);
                                  ...
                                  PyObject *kernreq_obj, PyObject *ectx_obj)
{
  try {
    ...
    std::string kr = pydynd::pystring_as_string(kernreq_obj);
