...
class TestCTypesDTypeInterop(unittest.TestCase):
    ...
    def test_dtype_from_ctypes_carray(self):
        self.assertEqual(ndt.make_fixedarray_dtype(ndt.int32, 10),
                nd.dtype(ctypes.c_int32 * 10))
        self.assertEqual(ndt.make_fixedarray_dtype(ndt.int32, (10, 3)),
                nd.dtype((ctypes.c_int32 * 3) * 10))
        self.assertEqual(ndt.make_fixedarray_dtype(ndt.int32, (10, 3, 4)),
                nd.dtype(((ctypes.c_int32 * 4) * 3) * 10))

        class POINT(ctypes.Structure):
            _fields_ = [('x', ctypes.c_int32), ('y', ctypes.c_int32)]
        self.assertEqual(ndt.make_fixedarray_dtype(nd.dtype(POINT), 10),
                nd.dtype(POINT * 10))
