...
class TestPythonScalar(unittest.TestCase):
    ...
    def test_utf_encodings(self):
        ...
        # UTF-8
        a = nd.array(x)
        a = a.ucast(ndt.make_fixedstring_type(16, 'utf_8'))
        a = a.eval()
        self.assertEqual(a.dtype, ndt.make_fixedstring_type(16, 'utf_8'))
        self.assertEqual(type(nd.as_py(a)), unicode)
        self.assertEqual(nd.as_py(a), x)
        # UTF-16
        a = nd.array(x)
        a = a.ucast(ndt.make_fixedstring_type(8, 'utf_16'))
        a = a.eval()
        self.assertEqual(a.dtype, ndt.make_fixedstring_type(8, 'utf_16'))
        self.assertEqual(type(nd.as_py(a)), unicode)
        self.assertEqual(nd.as_py(a), x)
        # UTF-32
        a = nd.array(x)
        a = a.ucast(ndt.make_fixedstring_type(8, 'utf_32'))
        a = a.eval()
        self.assertEqual(a.dtype, ndt.make_fixedstring_type(8, 'utf_32'))
        self.assertEqual(type(nd.as_py(a)), unicode)
        self.assertEqual(nd.as_py(a), x)
