                          ...
                          const eval::eval_context *DYND_UNUSED(ectx))
{
    ...
    const date_type *dd = tp.extended<date_type>();
                          ...
                          const eval::eval_context *DYND_UNUSED(ectx))
{
    ...
    const time_type *tt = tp.extended<time_type>();
    tt->set_time(
                              ...
                              const eval::eval_context *DYND_UNUSED(ectx))
{
    ...
    const datetime_type *dd = tp.extended<datetime_type>();
...
{
    if (obj == Py_None) {
        tp.extended<option_type>()->assign_na(arrmeta, out,
        ...
        if (element_tp.is_scalar()) {
            ...
            for (Py_ssize_t i = 0; i < size; ++i) {
                fill_array_from_pylist<ConvertOneFn>(
                    element_tp, element_arrmeta, element_data,
                    PyList_GET_ITEM(obj, i), shape, current_axis + 1, ectx);
                element_data += stride;
            }
        }
    }
                                         ...
                                         const eval::eval_context *ectx)
{
    ...
    switch (tp.get_type_id()) {
        ...
        case bytes_type_id:
            ...
            break;
        case string_type_id: {
            const base_string_type *ext = tp.extended<base_string_type>();
...
# if SIZEOF_LONG > SIZEOF_INT
    ...
    // external memory
    Py_INCREF(obj);
    memory_block_ptr bytesref = make_external_memory_block(
    ...
    const datetime_type *dd = d.extended<datetime_type>();
    result = nd::empty(d);
    ...
    const date_type *dd = d.extended<date_type>();
    result = nd::empty(d);
    ...
    const time_type *tt = d.extended<time_type>();
    result = nd::empty(d);
