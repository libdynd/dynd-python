...
namespace {
    struct pyfunc_expr_ck : public kernels::general_ck<pyfunc_expr_ck> {
        ...
        inline void init_kernfunc(kernel_request_t kernreq)
        {
            ...
            case kernel_request_single:
                base.template set_function<expr_single_operation_t>(&self_type::single);
                break;
            case kernel_request_strided:
                base.template set_function<expr_strided_operation_t>(&self_type::strided);
                break;
            default: {
                std::stringstream ss;
        ...
        uint32_t kernreq, const eval::eval_context *ectx)
    {
        ...
        PyGILState_RAII pgs;
        PyObject *pyfunc = reinterpret_cast<PyObject *>(af_self->data_ptr);
