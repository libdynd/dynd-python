...
intptr_t pydynd::nd::copy_from_numpy_kernel::instantiate(
    const dynd::arrfunc_type_data *self_af, const dynd::ndt::arrfunc_type *af_tp,
    ...
    const std::map<dynd::nd::string, dynd::ndt::type> &tp_vars)
{
  if (src_tp[0].get_type_id() != dynd::void_type_id) {
    stringstream ss;
    ss << "Cannot instantiate dynd::nd::arrfunc copy_from_numpy with signature ";
    ss << af_tp << " with types (";
    ss << src_tp[0] << ") -> " << dst_tp;
  ...
  if (!PyDataType_FLAGCHK(dtype, NPY_ITEM_HASOBJECT)) {
    ...
    dynd::ndt::type src_view_tp = ndt_type_from_numpy_dtype(dtype, src_alignment);
    return dynd::make_assignment_kernel(NULL, NULL, ckb, ckb_offset, dst_tp,
    ...
    if (dst_tp.get_kind() != dynd::struct_kind && dst_tp.get_kind() != dynd::tuple_kind) {
      stringstream ss;
    ...
    pydynd::extract_fields_from_numpy_struct(dtype, field_dtypes_orig, field_names_orig,
                                     field_offsets_orig);
    intptr_t field_count = field_dtypes_orig.size();
    if (field_count != dst_tp.extended<dynd::ndt::base_tuple_type>()->get_field_count()) {
      stringstream ss;
      ss << "Cannot assign from numpy type " << pyobject_repr((PyObject *)dtype)
         << " to dynd type " << dst_tp;
      throw invalid_argument(ss.str());
    ...
    if (dst_tp.get_kind() == dynd::struct_kind) {
      ...
      for (intptr_t i = 0; i < field_count; ++i) {
        intptr_t src_i = dst_tp.extended<dynd::ndt::base_struct_type>()->get_field_index(
            field_names_orig[i]);
        if (src_i >= 0) {
    ...
    return make_tuple_unary_op_ckernel(
        ...
        dst_tp.extended<dynd::ndt::base_tuple_type>()->get_data_offsets(dst_arrmeta),
...
{
  return dynd::nd::functional::elwise(dynd::nd::arrfunc::make<copy_from_numpy_kernel>(
                                       ...
                                       const dynd::eval::eval_context *ectx)
{
  ...
  dynd::ndt::type src_tp =
      dynd::ndt::make_type(src_ndim, PyArray_SHAPE(src_arr), dynd::ndt::make_type<void>());
