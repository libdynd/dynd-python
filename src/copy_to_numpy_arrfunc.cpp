    ...
    const dynd::nd::array &kwds, const std::map<dynd::nd::string, dynd::ndt::type> &tp_vars)
{
  ...
  if (!PyDataType_FLAGCHK(dtype, NPY_ITEM_HASOBJECT)) {
    ...
    dynd::ndt::type dst_view_tp = ndt_type_from_numpy_dtype(dtype, dst_alignment);
    return dynd::make_assignment_kernel(NULL, NULL, ckb, ckb_offset, dst_view_tp,
                                  NULL, src_tp[0], src_arrmeta[0], kernreq,
                                  ectx, dynd::nd::array());
  } else if (PyDataType_ISOBJECT(dtype)) {
    const dynd::arrfunc_type_data *af =
    ...
    vector<dynd::ndt::type> dst_fields_tp(field_count, dynd::ndt::make_type<void>());
    ...
    return make_tuple_unary_op_ckernel(
        ...
        src_tp[0].extended<dynd::ndt::base_tuple_type>()->get_data_offsets(src_arrmeta[0]),
...
dynd::nd::arrfunc pydynd::copy_to_numpy::make()
{
  return dynd::nd::functional::elwise(
      dynd::nd::arrfunc::make<copy_to_numpy_ck>(dynd::ndt::type("(Any) -> void"), 0));
}
                                 ...
                                 const dynd::eval::eval_context *ectx)
{
  ...
  dynd::ndt::type dst_tp =
      dynd::ndt::make_type(dst_ndim, PyArray_SHAPE(dst_arr), dynd::ndt::make_type<void>());
  ...
  intptr_t tmp_dst_arrmeta_size =
      dst_ndim * sizeof(dynd::fixed_dim_type_arrmeta) + sizeof(copy_to_numpy_arrmeta);
