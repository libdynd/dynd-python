                                             ...
                                             size_t data_alignment)
{
  ...
  for (size_t i = 0; i < field_dtypes.size(); ++i) {
    ...
    field_types.push_back(pydynd::ndt_type_from_numpy_dtype(fld_dtype, data_alignment));
                                            ...
                                            size_t data_alignment)
{
  ...
  case NPY_CDOUBLE:
    ...
    break;
  case NPY_STRING:
    dt = dynd::ndt::make_fixed_string(d->elsize, dynd::string_encoding_ascii);
    break;
  case NPY_UNICODE:
    dt = dynd::ndt::make_fixed_string(d->elsize / 4, dynd::string_encoding_utf_32);
    break;
  case NPY_VOID:
  ...
  case NPY_DATETIME: {
    ...
    if (s == "D") {
      ...
      // metadata
      dt = dynd::ndt::make_adapt(dynd::ndt::make_type<int64_t>(), dynd::ndt::make_date(),
                           "days since 1970");
    } else if (s == "h") {
      dt = dynd::ndt::make_adapt(dynd::ndt::make_type<int64_t>(),
                           dynd::ndt::make_datetime(dynd::tz_utc), "hours since 1970");
    } else if (s == "m") {
      dt = dynd::ndt::make_adapt(dynd::ndt::make_type<int64_t>(),
                           dynd::ndt::make_datetime(dynd::tz_utc), "minutes since 1970");
    } else if (s == "s") {
      dt = dynd::ndt::make_adapt(dynd::ndt::make_type<int64_t>(),
                           dynd::ndt::make_datetime(dynd::tz_utc), "seconds since 1970");
    } else if (s == "ms") {
      dt =
          dynd::ndt::make_adapt(dynd::ndt::make_type<int64_t>(), dynd::ndt::make_datetime(dynd::tz_utc),
...
PyArray_Descr *pydynd::numpy_dtype_from_ndt_type(const dynd::ndt::type& tp)
{
    switch (tp.get_type_id()) {
        ...
        case tuple_type_id: {
            ...
            const vector<dynd::ndt::type>& fields = ttp->get_fields();
        ...
        case dynd::struct_type_id: {
            ...
            for (size_t i = 0; i < field_count; ++i) {
                const dynd::string_type_data& fname = ttp->get_field_name_raw(i);
#if PY_VERSION_HEX >= 0x03000000
...
dynd::nd::array pydynd::array_from_numpy_scalar(PyObject* obj, uint32_t access_flags)
{
    dynd::nd::array result;
    if (PyArray_IsScalar(obj, Bool)) {
        result = dynd::nd::array((dynd::dynd_bool)(((PyBoolScalarObject *)obj)->obval != 0));
    } else if (PyArray_IsScalar(obj, Byte)) {
        ...
        if (scalar->obmeta.base <= NPY_FR_D) {
            ...
            *reinterpret_cast<int32_t *>(result.get_readwrite_originptr()) =
            ...
            result = dynd::nd::empty(dynd::ndt::make_datetime(dynd::tz_utc));
            int64_t result_val;
            switch (scalar->obmeta.base) {
            case NPY_FR_h:
