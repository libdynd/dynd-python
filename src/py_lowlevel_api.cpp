...
PyObject *make_arrfunc_from_property(PyObject *tp_obj, PyObject *propname_obj)
{
  try {
    ...
    dynd::nd::arrfunc af = ::make_arrfunc_from_property(tp, propname);
...
{
  try {
    return wrap_array(pydynd::nd::functional::apply(pyfunc, proto));
  }
  catch (...) {
    translate_exception();
    return NULL;
...
PyObject *make_builtin_mean1d_arrfunc(PyObject *tp_obj, PyObject *minp_obj)
{
  try {
    ...
    return wrap_array(
        kernels::make_builtin_mean1d_arrfunc(tp.get_type_id(), minp));
  }
  catch (...) {
    translate_exception();
    return NULL;
