...
void pydynd::vm_elwise_program_from_py(PyObject *obj,
                                       dynd::vm::elwise_program &out_ep)
{
  ...
  for (Py_ssize_t i = 0; i < program_size; ++i) {
    ...
    if (instr_size == 0) {
      throw runtime_error(
          "Each instruction in the VM program must have at least an opcode");
    }
    // The opcode
...
PyObject *pydynd::vm_elwise_program_as_py(dynd::vm::elwise_program &ep)
{
  ...
  for (int i = 0; i < ep.get_instruction_count(); ++i) {
    ...
    pyobject_ownref name_str(
        PyUnicode_FromString(vm::opcode_info[opcode].name));
#else
    ...
    for (int j = 1; j < 2 + arity; ++j) {
      PyTuple_SET_ITEM(instr.get(), j,
                       PyLong_FromLong(ep.get_program()[ip + j]));
    }
